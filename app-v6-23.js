...
function bindPinPad(){
 const buttons=document.querySelectorAll('.pinDirect');
 buttons.forEach(btn=>{
   ...
   btn.addEventListener('click',(e)=>{
     ...
     if(btn.dataset.bio==='1'){
       pinBio();
       return;
     }

     const v=btn.dataset.pin;
     if(v!==undefined)pinKey(v);
   },{passive:false});
 });
}
function render(){
...
function pinSetupScreen(){
 ...
 return `<div class="premiumLock setupPremiumLock">
   ...
   <div class="pinPad setupPinPad">
     ${[1,2,3,4,5,6,7,8,9].map(n=>`<button type="button" class="setupPinBtn" onclick="setupPinKey('${n}')">${n}</button>`).join('')}
     <button type="button" class="setupPinGhost" disabled>◎</button>
     <button type="button" class="setupPinBtn" onclick="setupPinKey('0')">0</button>
     <button type="button" class="setupPinBtn delKey" onclick="setupPinKey('del')">⌫</button>
   </div>

   <button type="button" class="setupPinOk" onclick="confirmSetupPin()">✓ TAMAM</button>
   <div class="setupPinNote">4 HANELİ PINİNİ GİRİP TAMAM'A BAS. SONRA AYNI PINİ TEKRAR GİR.</div>
   <div class="lockVersion">V6.23</div>
 </div>`;
}
function createInitialPin(){
 // V6.23: PIN oluşturma artık ekran tuş takımı üzerinden yapılır.
}
let pinBuffer='';
...
function lockScreen(){
 ...
 return `<div class="premiumLock">
   ...
   <div class="lockQuote">“Bugün yapabileceklerinin en iyisini yap; yarın senin için daha güzel olacak.”</div><div class="lockVersion">V6.21</div>
 </div>`;
}
function unlockApp(){
