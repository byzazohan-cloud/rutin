...
function pinKey(v){
  ...
  if(!/^\d$/.test(val) || pinBuffer.length>=4)return false;
...
function lockScreen(){
 ...
 return `<div class="premiumLock">
   ...
   <div class="pinPad">
    ${[1,2,3,4,5,6,7,8,9].map(n=>`<button type="button" class="pinBtn" data-pin="${n}">${n}</button>`).join('')}
    <button type="button" class="bioKey pinBtn" data-bio="1">◎</button><button type="button" class="pinBtn" data-pin="0">0</button><button type="button" class="delKey pinBtn" data-pin="del">⌫</button>
   </div>
