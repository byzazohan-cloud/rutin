...
function pinSetupScreen(){
 return `<div class="premiumLock pinSetup">
   <div class="lockTop">
     <div></div>
     <div class="lockBrand">${rutinLogo(42)}<strong>RUTİN</strong><small>İLK KURULUM</small></div>
     <div></div>
   </div>
   <div class="pinSetupCard">
     <div class="setupShield">⌾</div>
     <h1>UYGULAMA KİLİDİ</h1>
     <p>RUTİN'İ KORUMAK İÇİN 4 HANELİ PIN BELİRLE</p>
     <input id="newPin1" type="password" inputmode="numeric" maxlength="4" placeholder="••••">
     <input id="newPin2" type="password" inputmode="numeric" maxlength="4" placeholder="PIN TEKRAR">
     <button class="primary" onclick="createInitialPin()">PIN OLUŞTUR</button>
   </div>
 </div>`;
}
function createInitialPin(){
 const a=$('#newPin1')?.value||'',b=$('#newPin2')?.value||'';
 if(!/^\d{4}$/.test(a))return alert('PIN 4 HANELİ OLMALI.');
 if(a!==b)return alert('PINLER AYNI DEĞİL.');
 state.settings.lock=true;
  state.settings.pin=a;
 save();
 unlocked=true;
