...
function home(){
 ...
 <div class="quick quickPremium quickSix">
   ...
   <button onclick="go('notes')"><i>✦</i><span>NOTLAR</span></button>
   <button onclick="go('investments')"><i>◇</i><span>YATIRIM</span></button>
...
function finance(){
 ...
 <button onclick="openModal('expense')"><i>🧾</i>HARCAMA</button><button onclick="openModal('income')"><i>₺＋</i>GELİR</button><button onclick="openModal('cash')"><i>₺</i>NAKİT</button><button onclick="go('investments')"><i>◆</i>YATIRIM</button>
 </div>`;
}
...
function reports(){
 ...
 <div class="section"><b>GELİR / HARCAMA GRAFİĞİ</b><span>GRAFİĞE DOKUN</span></div>
 <div class="card clickableChart" onclick="go('detail')">
   <div class="chartBars">
   ...
   <div class="legend"><span><i style="background:#4bb874"></i>GELİR</span><span><i style="background:#d8575c"></i>HARCAMA</span></div>
   <div class="chartTap">AYRINTILI RAPORU AÇ ›</div>
 </div>`;
}
...
function profile(){
 ...
 <div class="profileShowcase">
   ...
   <div class="profileBadge">RUTİN · KİŞİSEL PANEL</div>
 </div>
 <div class="card">
...
function expenseForm(){return `<form onsubmit="submitExpense(event)">
 ...
 <div class="field"><label>HESAPTAN</label>
  <div class="payIcons">
    ...
    <label><input type="radio" name="method" value="ESNEK HESAP"><i>▥</i><span>ESNEK HESAP</span></label>
  </div>
 </div>
 ${field('date','TARİH',iso(),'date')}
...
function render(){
 ...
 <div class="moreGrid">
   ...
   <button onclick="openModal('expense')"><i>🧾</i><b>KAYIT EKLE</b><span>GELİR / HARCAMA</span></button>
 </div>`;
...
function modalHtml(k){
 ...
 if(k.startsWith('day:')){const ds=k.split(':')[1],items=state.work.filter(x=>x.date===ds);title=`${ds} · GÜN KAYDI`;body=`<div class="notice">BU GÜNE YENİ ÇALIŞMA EKLEYEBİLİR VE MEVCUT KAYITLARI SİLEBİLİRSİN.</div><div class="quick" style="grid-template-columns:repeat(3,1fr)"><button onclick="closeModal();setTimeout(()=>openModal('dailyDate:${ds}'),0)"><i>✓</i>GÜNLÜK</button><button onclick="closeModal();setTimeout(()=>openModal('hourlyDate:${ds}'),0)"><i>◷</i>SAATLİK</button><button onclick="closeModal();setTimeout(()=>openModal('overtimeDate:${ds}'),0)"><i>✦</i>MESAİ</button></div><div class="card list" style="margin-top:10px">${items.length?items.map(x=>`<div class="item"><div class="ico">${x.type==='daily'?'✓':x.type==='hourly'?'◷':'✦'}</div><div><b>${x.title}</b><small>${x.hours?x.hours+' SAAT':''}</small></div><button type="button" class="miniDelete" onclick="deleteWork('${x.id}','${ds}')">SİL</button></div>`).join(''):'<div class="notice">BU GÜNDE KAYIT YOK.</div>'}</div>`}
