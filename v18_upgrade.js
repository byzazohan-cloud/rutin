...
function expenseCategoryV4313(x){
 const cats=(state.categories||[]).map(c=>upper(String(c||'').trim()));
 const raw=upper(String(x?.category||'').trim());
 const title=upper(String(x?.title||'').trim());
 const custom=upper(String(x?.customTitle||'').trim());
 if(raw && raw!=='DİĞER') return raw;
 if(cats.includes(title) && title!=='DİĞER') return title;
 if(raw==='DİĞER') return 'DİĞER';
 if(cats.includes(custom) && custom!=='DİĞER') return custom;
 return raw||'DİĞER';
}
function expenseDisplayV4313(x){
 const c=expenseCategoryV4313(x);
 if(c!=='DİĞER') return c;
 const d=upper(String(x?.customTitle||((x?.title&&upper(String(x.title))!=='DİĞER')?x.title:'')||x?.note||'').trim());
 return d?`DİĞER — ${d}`:'DİĞER';
}
window.expenseCategoryV4313=expenseCategoryV4313;
window.expenseDisplayV4313=expenseDisplayV4313;
let navStack=[];
const rawGo=go;
...
function expenseRowV24(x){
 const title=window.esc(expenseDisplayV4313(x)),cat=window.esc(expenseCategoryV4313(x)),method=window.esc(x&&x.method||x&&x.paymentMethod||'NAKİT'),date=window.esc(x&&x.date||'');
...
expenseAnalytics=function(){
 ...
 mm.forEach(x=>{const c=expenseCategoryV4313(x);totals[c]=(totals[c]||0)+(Number(x&&x.amount)||0)});
