...
const defaults={
 ...
 settings:{dailyRate:1250,hourlyRate:200,overtimeRate:250,pin:'',lock:true,reminders:true,reminderDays:2,morningReminder:true,morningReminderTime:'07:00',nightReminder:true,nightReminderTime:'22:00',appearance:'dark',theme:{bg:'#000000',panel:'#000000',gold:'#d7b45c',green:'#22c55e',red:'#ff4d5a',blue:'#3b82f6'}},
...
function submitEditWork(e,id,ds){
 e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries());
 const x=state.work.find(z=>z.id===id);if(!x)return;
 x.date=d.date||x.date;
 x.title=upper(d.title||x.title||'ÇALIŞMA');
 x.note=d.note||'';
 if(x.type==='daily'){
   x.amount=+d.amount||0;
 }else{
...
function pinSetupScreen(){
 return `<div class="premiumLock pinSetup">
   <div class="lockTop">
     <div></div>
     <div class="lockBrand"><div class="lockLogo">✓</div><strong>RUTİN</strong><small>İLK KURULUM</small></div>
     <div></div>
   </div>
...
function lockScreen(){
 ...
 return `<div class="premiumLock">
   ...
   <div class="lockTop">
     ...
     <div class="lockBrand"><div class="lockLogo">✓</div><strong>RUTİN</strong><small>DAHA İYİ BİR SEN</small></div>
   ...
   <div class="pinPad">
    ...
    <button class="bioKey" onclick="pinBio()">◎</button><button onclick="pinKey('0')">0</button><button class="delKey" onclick="pinKey('del')">⌫</button>
   </div>
   <div class="lockBottom"><span>▣ PAROLAMI UNUTTUM</span><b>KÜÇÜK ADIMLAR<br>BÜYÜK SONUÇLAR</b><span>⌾ GÜVENLİ GİRİŞ</span></div>
   <div class="lockQuote">“Bugün yapabileceklerinin en iyisini yap; yarın senin için daha güzel olacak.”</div>
 </div>`;
}
function unlockApp(){
