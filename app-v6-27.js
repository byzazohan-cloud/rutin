...
let nativePinStep='login';
let nativePinFirst='';

function nativePinScreen(){
  if(!state.settings.pin && nativePinStep==='login')nativePinStep='create';
  if(state.settings.pin && nativePinStep!=='login')nativePinStep='login';

  const name=state.profile?.name||'RUTİN';
  const photo=state.profile?.photo||'';
  const instruction=nativePinStep==='create'?'YENİ 4 HANELİ PINİNİ GİR':nativePinStep==='confirm'?'PINİ TEKRAR GİR':'4 HANELİ PINİNİ GİR';

  return `<div class="premiumLock nativePinScreen">
    ...
    <div class="nativePinInstruction">${instruction}</div>

    <form class="nativePinForm" onsubmit="submitNativePin(event)">
      <input id="nativePinInput" name="pin" type="tel" inputmode="numeric" pattern="[0-9]*" maxlength="4" autocomplete="off" enterkeyhint="done" placeholder="••••" aria-label="4 haneli PIN">
      <button type="submit" class="nativePinOk">✓ TAMAM</button>
    </form>
    ...
    <div class="lockVersion">V6.27</div>
  </div>`;
}

function submitNativePin(e){
  e.preventDefault();
  const input=document.getElementById('nativePinInput');
  const pin=String(input?.value||'').replace(/[^0-9]/g,'').slice(0,4);
  ...
  if(nativePinStep==='create'){
    nativePinFirst=pin;
    nativePinStep='confirm';
    if(input){input.value='';input.focus();}
    const inst=document.querySelector('.nativePinInstruction');
    if(inst)inst.textContent='PINİ TEKRAR GİR';
    return;
  }

  if(nativePinStep==='confirm'){
    if(pin!==nativePinFirst){
      nativePinFirst='';
      nativePinStep='create';
      if(input){input.value='';input.focus();}
      const inst=document.querySelector('.nativePinInstruction');
      if(inst)inst.textContent='YENİ 4 HANELİ PINİNİ GİR';
...
function submitSecurity(e){
 ...
 modal=null;
 unlocked=false;
 nativePinStep=state.settings.pin?'login':'create';
 nativePinFirst='';
 render();
}
