...
document.addEventListener('visibilitychange',()=>{
  ...
  if(document.visibilityState==='visible' && state.settings.lock && hiddenAt){
    const awayMs=Date.now()-hiddenAt;
    hiddenAt=0;
    // Mobile keyboards can briefly alter visibility/focus.
    // Only a genuine background period relocks the current session.
    if(awayMs>=15000){
      unlocked=false;
      modal=null;
      render();
