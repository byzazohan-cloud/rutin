...
const defaults={
 ...
 settings:{dailyRate:1250,hourlyRate:200,overtimeRate:250,pin:'',lock:false,reminders:true,reminderDays:2,theme:{bg:'#000000',panel:'#000000',gold:'#d7b45c',green:'#22c55e',red:'#ff4d5a',blue:'#3b82f6'}},
...
function calendarScreen(){
 ...
 for(let n=1;n<=days;n++){
  ...
  cells+=`<button class="day ${cl}${isToday}" onclick="openModal('day:${ds}')">${n}</button>`;
 }
 const W=workSummary();
 const monthWork=monthItems(state.work);
 ...
 <div class="calendarTitleRow">
   <button class="calendarArrow" aria-label="Önceki ay">‹</button>
   <div><b>${d.toLocaleDateString('tr-TR',{month:'long',year:'numeric'}).toUpperCase()}</b><small>GÜNE DOKUNARAK KAYIT EKLE / DÜZENLE</small></div>
   <button class="calendarArrow" aria-label="Sonraki ay">›</button>
 </div>
