...
function bindPinPad(){
 document.querySelectorAll('.pinDirect').forEach(btn=>{
   if(btn.dataset.bound==='1')return;
   btn.dataset.bound='1';
   btn.addEventListener('click',(e)=>{
     e.preventDefault();e.stopPropagation();
     if(btn.dataset.bio==='1'){pinBio();return;}
     if(btn.dataset.pin!==undefined)pinKey(btn.dataset.pin);
   },{passive:false});
 });

 document.querySelectorAll('.setupPinDirect').forEach(btn=>{
   if(btn.dataset.bound==='1')return;
   btn.dataset.bound='1';
   btn.addEventListener('click',(e)=>{
     e.preventDefault();e.stopPropagation();
     if(btn.dataset.setupPin!==undefined)setupPinKey(btn.dataset.setupPin);
   },{passive:false});
 });

 const ok=document.querySelector('[data-setup-ok="1"]');
...
function handlePinKeyboard(e){
 ...
 const key=e.key;
 const setup=!state.settings.pin;
 if(/^\d$/.test(key)){
   e.preventDefault();
   setup?setupPinKey(key):pinKey(key);
   return;
 }
 if(key==='Backspace' || key==='Delete'){
   e.preventDefault();
   setup?setupPinKey('del'):pinKey('del');
   return;
 }
 if(key==='Enter' && setup){
   e.preventDefault();
...
function render(){
 if(state.settings.lock && !unlocked){
   $('#app').innerHTML=state.settings.pin?lockScreen():pinSetupScreen();
   requestAnimationFrame(()=>bindPinPad());
   return;
 }
 let content='';
...
function updateSetupPinDots(){
 const dots=document.querySelectorAll('.setupPinDots i');
 dots.forEach((dot,i)=>dot.classList.toggle('filled',setupPinBuffer.length>i));
 const label=document.querySelector('.setupPinInstruction');
 if(label){
   label.textContent=setupPinStep===1?'YENİ 4 HANELİ PINİNİ GİR':'PINİ TEKRAR GİR';
 }
}

function setupPinKey(v){
 const val=String(v);
 if(val==='del'){
 ...
 if(!/^\d$/.test(val) || setupPinBuffer.length>=4)return;
 setupPinBuffer+=val;
 updateSetupPinDots();
}
...
function confirmSetupPin(){
 ...
 state.settings.lock=true;
 state.settings.securityBootstrapVersion=1;
 state.settings.pin=setupPinBuffer;
 save();
...
function pinSetupScreen(){
 ...
 return `<div class="premiumLock setupPremiumLock">
   <div class="lockAura"></div>

   <div class="lockTop">
     <div class="lockSideQuote">DİSİPLİN<br>BUGÜNÜ<br>YARINA TAŞIR</div>
     <div class="lockBrand">${rutinLogo(42)}<strong>RUTİN</strong><small>İLK KURULUM</small></div>
   ...
   <div class="setupPinTitle">UYGULAMA KİLİDİ</div>
   <div class="setupPinInstruction">${setupPinStep===1?'YENİ 4 HANELİ PINİNİ GİR':'PINİ TEKRAR GİR'}</div>

   <div class="pinDots setupPinDots">
     ${[0,1,2,3].map((_,i)=>`<i class="${setupPinBuffer.length>i?'filled':''}"></i>`).join('')}
   </div>

   <div class="pinPad setupPinPad">
     ${[1,2,3,4,5,6,7,8,9].map(n=>`<button type="button" class="setupPinBtn setupPinDirect" data-setup-pin="${n}">${n}</button>`).join('')}
     <button type="button" class="setupPinGhost" disabled>◎</button>
     <button type="button" class="setupPinBtn setupPinDirect" data-setup-pin="0">0</button>
     <button type="button" class="setupPinBtn delKey setupPinDirect" data-setup-pin="del">⌫</button>
   </div>

   <button type="button" class="setupPinOk" data-setup-ok="1">✓ TAMAM</button>
   <div class="setupPinNote">4 HANELİ PINİNİ GİRİP TAMAM'A BAS. SONRA AYNI PINİ TEKRAR GİR.</div>
   <div class="lockVersion">V6.24</div>
 </div>`;
}
function createInitialPin(){
 // V6.24: PIN oluşturma artık ekran tuş takımı üzerinden yapılır.
}
let pinBuffer='';
function updatePinDots(){
 const dots=document.querySelectorAll('.pinDots i');
 dots.forEach((dot,i)=>dot.classList.toggle('filled',pinBuffer.length>i));
}
function pinKey(v){
 const val=String(v);
 ...
 if(!/^\d$/.test(val) || pinBuffer.length>=4)return;

 pinBuffer+=val;
 updatePinDots();

 if(pinBuffer.length===4){
   if(pinBuffer===String(state.settings.pin||'')){
     pinBuffer='';
     unlocked=true;
...
function lockScreen(){
 const name=state.profile?.name||'RUTİN';
 const photo=state.profile?.photo||'';
 return `<div class="premiumLock">
   <div class="lockAura"></div>
   <div class="lockTop">
     <div class="lockSideQuote">DİSİPLİN<br>BUGÜNÜ<br>YARINA TAŞIR</div>
     <div class="lockBrand">${rutinLogo(42)}<strong>RUTİN</strong><small>DAHA İYİ BİR SEN</small></div>
     <div class="lockSideQuote right">PLANLA<br>ÇALIŞ<br>BAŞAR</div>
   </div>
   <div class="lockProfile">${photo?`<img src="${photo}" alt="">`:`<div class="lockAvatar">${name.slice(0,1).toUpperCase()}</div>`}</div>
   <h1>${name}</h1>
   <div class="lockSub">RUTİN SENİNLE DAHA GÜÇLÜ</div>
   <div class="lockGreeting">${greetingByTime()}!</div>
   <div class="lockGreetingSub">${dailyQuote()}</div>
   <div class="pinDots">${[0,1,2,3].map((_,i)=>`<i class="${pinBuffer.length>i?'filled':''}"></i>`).join('')}</div>
   <div class="pinPad">
    ${[1,2,3,4,5,6,7,8,9].map(n=>`<button type="button" class="pinDirect" data-pin="${n}">${n}</button>`).join('')}
    <button type="button" class="bioKey pinDirect" data-bio="1">◎</button><button type="button" class="pinDirect" data-pin="0">0</button><button type="button" class="delKey pinDirect" data-pin="del">⌫</button>
   </div>
   <div class="lockBottom"><button type="button" class="forgotPinBtn" onclick="resetRutinPin()">▣ PAROLAMI UNUTTUM</button><b>KÜÇÜK ADIMLAR<br>BÜYÜK SONUÇLAR</b><span>⌾ GÜVENLİ GİRİŞ</span></div>
   <div class="lockQuote">“Bugün yapabileceklerinin en iyisini yap; yarın senin için daha güzel olacak.”</div><div class="lockVersion">V6.24</div>
 </div>`;
}
function unlockApp(){
 const v=$('#pinUnlock')?.value||'';
...
function resetRutinPin(){
 const ok=confirm("RUTİN PIN'İ SIFIRLANSIN MI?\n\nBu işlem yalnızca RUTİN'in kendi kayıtlarını sıfırlar. HANE verilerine dokunmaz.");
 if(!ok)return;

 // Yalnızca RUTİN'e ait depolama anahtarlarını sil.
 [STORAGE_KEY,'rutin-v4','rutin-v3','rutin-v2','rutin-v1'].forEach(k=>{
   try{localStorage.removeItem(k)}catch(e){}
 });

 // Clear only RUTİN caches, not the entire origin cache namespace.
 if('caches' in window){
   caches.keys().then(keys=>{
     keys.filter(k=>k.startsWith('rutin-')).forEach(k=>caches.delete(k));
   }).catch(()=>{});
 }

 alert("RUTİN PIN SIFIRLANDI. ŞİMDİ YENİ PIN OLUŞTURABİLİRSİN.");
 location.reload();
}
function pinBio(){alert('BİYOMETRİK GİRİŞ BU WEB SÜRÜMÜNDE CİHAZ DESTEĞİ GEREKTİRİR. PIN İLE DEVAM EDEBİLİRSİN.');}
