...
function ensurePinFlow(){
  if(pinFlow.mode===null){
    pinFlow.mode=state.settings.pin?'login':'create';
    pinFlow.buffer='';
    pinFlow.first='';
  }
}

function pinFlowInstruction(){
  if(pinFlow.mode==='create') return 'YENİ 4 HANELİ PINİNİ GİR';
  if(pinFlow.mode==='confirm') return 'PINİ TEKRAR GİR';
  return '4 HANELİ PINİNİ GİR';
}

function pinFlowScreen(){
  ensurePinFlow();
  ...
  return `<div class="premiumLock unifiedPinScreen">
    ...
    <div class="pinInstruction">${pinFlowInstruction()}</div>

    <div class="pinDots">
      ${[0,1,2,3].map((_,i)=>`<i class="${pinFlow.buffer.length>i?'filled':''}"></i>`).join('')}
    </div>

    <div class="pinPad fixedPinPad">
      ${[1,2,3,4,5,6,7,8,9].map(n=>`<button type="button" class="fixedPinBtn" data-pin="${n}">${n}</button>`).join('')}
      <button type="button" class="fixedPinBtn pinGhost" data-pin="clear">C</button>
      <button type="button" class="fixedPinBtn" data-pin="0">0</button>
      <button type="button" class="fixedPinBtn" data-pin="del">⌫</button>
    </div>

    <button type="button" class="pinOkBtn fixedPinOk">✓ TAMAM</button>

    ${pinFlow.mode==='login'
      ? `<button type="button" class="forgotPinBtn nativeForgot" onclick="resetRutinPin()">PAROLAMI UNUTTUM</button>`
      : `<div class="nativePinHelp">4 HANELİ PINİNİ GİRİP TAMAM'A BAS.</div>`}

    <div class="lockVersion">V6.28</div>
  </div>`;
}

function updatePinFlowUI(){
  const dots=document.querySelectorAll('.pinDots i');
  dots.forEach((dot,i)=>dot.classList.toggle('filled',pinFlow.buffer.length>i));

  const inst=document.querySelector('.pinInstruction');
  if(inst) inst.textContent=pinFlowInstruction();
}

function pinFlowKey(value){
  ...
  if(!/^\d$/.test(v) || pinFlow.buffer.length>=4) return;

  pinFlow.buffer+=v;
  updatePinFlowUI();
}

function confirmPinFlow(){
  if(pinFlow.buffer.length!==4){
    alert('LÜTFEN 4 HANELİ PIN GİR.');
    return;
  }

  if(pinFlow.mode==='create'){
    pinFlow.first=pinFlow.buffer;
    pinFlow.buffer='';
    pinFlow.mode='confirm';
    updatePinFlowUI();
    return;
  }

  if(pinFlow.mode==='confirm'){
    if(pinFlow.buffer!==pinFlow.first){
      pinFlow={mode:'create',buffer:'',first:''};
      updatePinFlowUI();
    ...
    state.settings.lock=true;
    state.settings.pin=pinFlow.buffer;
    save();

    pinFlow={mode:'login',buffer:'',first:''};
    unlocked=true;
    render();
    return;
  }
  ...
  // login
  if(pinFlow.buffer===String(state.settings.pin||'')){
    pinFlow.buffer='';
    unlocked=true;
    render();
    return;
...
function bindPinFlowControls(){
  document.querySelectorAll('.fixedPinBtn').forEach(btn=>{
    ...
    btn.addEventListener('click',(e)=>{
      e.preventDefault();
      e.stopPropagation();
      pinFlowKey(btn.dataset.pin);
    },{passive:false});
  });

  const ok=document.querySelector('.fixedPinOk');
...
function render(){
 if(state.settings.lock && !unlocked){
   $('#app').innerHTML=pinFlowScreen();
   requestAnimationFrame(()=>bindPinFlowControls());
   return;
 }
 let content='';
...
function submitSecurity(e){
 ...
 if(!wantLock){
   ...
   unlocked=true;
   modal=null;
   pinFlow={mode:null,buffer:'',first:''};
 ...
 modal=null;
 unlocked=false;
 pinFlow={mode:state.settings.pin?'login':'create',buffer:'',first:''};
 render();
}
