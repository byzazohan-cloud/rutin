...
if((state.settings.securityVersion||0)<2){
  state.settings.lock=true;
  state.settings.securityVersion=2;
  localStorage.setItem('rutin-v3',JSON.stringify(state));
}
...
document.addEventListener('visibilitychange',()=>{
  if(document.visibilityState==='hidden' && state.settings.lock){
    unlocked=false;
  }
});
applyTheme();
