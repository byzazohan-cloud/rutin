...
let pinState={
  mode:'login',      // login | create | confirm
  buffer:'',
  first:''
};

function getPinMode(){
  if(!state.settings.lock)return 'off';
  if(!state.settings.pin)return 'create';
  return 'login';
}

function preparePinMode(){
  const mode=getPinMode();
  if(mode==='create' && pinState.mode!=='create'){
    pinState={mode:'create',buffer:'',first:''};
  }else if(mode==='login' && pinState.mode!=='login'){
    pinState={mode:'login',buffer:'',first:''};
  }
}

function pinInstruction(){
  if(pinState.mode==='create')return 'YENİ 4 HANELİ PINİNİ GİR';
  if(pinState.mode==='confirm')return 'PINİ TEKRAR GİR';
...
function pinScreen(){
  ...
  return `<div class="premiumLock unifiedPinScreen">
    <div class="lockAura"></div>

    <div class="lockTop">
      <div class="lockSideQuote">DİSİPLİN<br>BUGÜNÜ<br>YARINA TAŞIR</div>
      <div class="lockBrand">${typeof rutinLogo==='function'?rutinLogo(42):'<div class="lockLogo">R✓</div>'}<strong>RUTİN</strong><small>DAHA İYİ BİR SEN</small></div>
    ...
    <div class="pinInstruction">${pinInstruction()}</div>

    <div class="pinDots">
      ${[0,1,2,3].map((_,i)=>`<i class="${pinState.buffer.length>i?'filled':''}"></i>`).join('')}
    </div>

    <div class="pinPad unifiedPinPad">
      ${[1,2,3,4,5,6,7,8,9].map(n=>`<button type="button" class="unifiedPinBtn" data-pin="${n}">${n}</button>`).join('')}
      <button type="button" class="unifiedPinBtn pinGhost" data-pin="bio">◎</button>
      <button type="button" class="unifiedPinBtn" data-pin="0">0</button>
      <button type="button" class="unifiedPinBtn" data-pin="del">⌫</button>
    </div>

    <button type="button" class="pinOkBtn" data-pin-ok="1">✓ TAMAM</button>

    <div class="pinHelp">
      ${pinState.mode==='login'
        ? `<button type="button" class="forgotPinBtn" onclick="resetRutinPin()">PAROLAMI UNUTTUM</button>`
        : `<span>4 HANELİ PINİNİ GİRİP TAMAM'A BAS.</span>`
      }
    </div>

    <div class="lockVersion">${version}</div>
  </div>`;
}

function updateUnifiedPinDots(){
  const dots=document.querySelectorAll('.pinDots i');
  dots.forEach((dot,i)=>dot.classList.toggle('filled',pinState.buffer.length>i));
  const inst=document.querySelector('.pinInstruction');
  if(inst)inst.textContent=pinInstruction();
}

function unifiedPinKey(v){
  const val=String(v);

  if(val==='bio'){
    alert('BİYOMETRİK GİRİŞ BU WEB SÜRÜMÜNDE CİHAZ DESTEĞİ GEREKTİRİR. PIN İLE DEVAM ET.');
    return;
  }

  if(val==='del'){
    pinState.buffer=pinState.buffer.slice(0,-1);
    updateUnifiedPinDots();
    return;
  }

  if(!/^\d$/.test(val) || pinState.buffer.length>=4)return;
  pinState.buffer+=val;
  updateUnifiedPinDots();
}

function confirmUnifiedPin(){
  ...
  if(pinState.mode==='create'){
    pinState.first=pinState.buffer;
    pinState.buffer='';
  ...
  if(pinState.mode==='confirm'){
    if(pinState.buffer!==pinState.first){
      pinState={mode:'create',buffer:'',first:''};
      updateUnifiedPinDots();
    ...
    state.settings.lock=true;
    state.settings.pin=pinState.buffer;
    save();
  ...
  if(pinState.mode==='login'){
    if(pinState.buffer===String(state.settings.pin||'')){
      pinState.buffer='';
      unlocked=true;
      render();
      return;
...
function bindUnifiedPinControls(){
  document.querySelectorAll('.unifiedPinBtn').forEach(btn=>{
    if(btn.dataset.bound==='1')return;
    btn.dataset.bound='1';
    btn.addEventListener('click',(e)=>{
      e.preventDefault();
      e.stopPropagation();
      unifiedPinKey(btn.dataset.pin);
    },{passive:false});
  });

  const ok=document.querySelector('[data-pin-ok="1"]');
...
// Fiziksel klavye / telefon klavyesi desteği
...
function render(){
 if(state.settings.lock && !unlocked){
   ...
   requestAnimationFrame(()=>bindUnifiedPinControls());
   return;
 }
 let content='';
...
function submitSecurity(e){
  ...
  modal=null;
  unlocked=false;
  pinState={mode:state.settings.pin?'login':'create',buffer:'',first:''};
  render();
}
